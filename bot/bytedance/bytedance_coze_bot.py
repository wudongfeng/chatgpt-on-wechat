...
class ByteDanceCozeBot(Bot):
    ...
    def reply(self, query, context=None):
        ...
        if context.type == ContextType.TEXT:
            ...
            logger.debug(f"[COZE] add reply to session success,reply_content={reply_content['content']}")
    ...
    def _reply_text(self, session_id: str, session: ChatGPTSession, retry_count=0):
        try:
            ...
            logger.debug(f"[COZE] chat_url={chat_url}; payload={payload}")
            response = requests.post(chat_url, headers=headers, json=payload)
            logger.debug(f"[COZE] response text={response.text} status_code={response.status_code}")
    ...
    def _convert_messages_format(self, messages) -> Tuple[str, List[dict]]:
        ...
        for message in messages:
            ...
            if role == 'user':
                ...
                # TODO: 为了避免机器人的回答token过长，可以采取压缩策略，例如：
                # 1.内容在n以内，直接放进去；
                # 2.内容超限制，直接进行摘要压缩，限制在n个字符内；
                # 3.直接放一个默认的值或者空，例如：“这是机器人的回复”
                content = message.get('content')
